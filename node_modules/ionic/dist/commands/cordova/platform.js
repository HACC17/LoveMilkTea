...
let PlatformCommand = class PlatformCommand extends base_1.CordovaCommand {
    ...
    run(inputs, options) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            const { RESOURCES, provideDefaultResources } = yield Promise.resolve().then(function () { return require('@ionic/cli-utils/lib/cordova/resources'); });
            ...
            if (action === 'add' && !(options['noresources']) && ['ios', 'android', 'wp8'].includes(platformName)) {
                yield provideDefaultResources(this.env, platformName);
                const conf = yield ConfigXml.load(this.env.project.directory);
                yield conf.ensurePlatformImages(platformName, RESOURCES[platformName]);
                yield conf.save();
            }
            this.env.tasks.end();
        });
